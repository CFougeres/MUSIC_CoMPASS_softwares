...
int BuildingEvent(int RunNumber, int file)
{
	...
	for(int t=0;t<10;t++){
        ...
        Tree_Event[t]->Branch("Cathode", &Cathode, "Cathode/D");   Tree_Event[t]->SetBranchAddress("Cathode",&Cathode);
        Tree_Event[t]->Branch("Grid", &Grid, "Grid/D");   Tree_Event[t]->SetBranchAddress("Grid",&Grid);
    ...
    P23=0.0;P67=0.0;Grid=0.0;Cathode=0.0;evt_time=0.; evt_mult=0;Si_dE=0;
    ...
    if(temp_index==NAnode+1){ Cathode=Energy;evt_mult+=1;}
    if(temp_index==NAnode+2){ Grid=Energy; evt_mult+=1; }
    ...
    for(int i = double(nStat)*double(compteur_tree*0.1)+1;i<run_portion*nStat;i++){
        ...
        if(diffTimeEvent>TimeWindow){
            ...
            P23=0.0;P67=0.0;Grid=0.0;Cathode=0.0;Si_dE=0;
